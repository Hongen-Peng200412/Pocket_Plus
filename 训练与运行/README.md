...
这些检查不代替服务器真实双节点 NCCL smoke。历史分支中的三个正式训练入口此前已经分别通过单节点训练启动验证；跨节点代码进入服务器正式使用前，还应使用两个节点执行短 smoke，核对每个 rank 的 hostname、全局 rank、NCCL 初始化、一次优化器更新、rank 0 checkpoint 和 `kill_lock` 清理。本次基础设施实现不提交新 Job，也不接管正在运行的 allocation。
