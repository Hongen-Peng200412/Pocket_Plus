...
# 使用冻结的 Find_1 F2 语义阈值和 Gaussian 参数, 为训练 PDB 的用户第 5 片生成 Stage2 和 Stage3 输入.
# 唯一合法位置参数是用户片号 5. 传给 CLI 前严格减一, 因此四个阶段均使用 --shard-index 4.
# probability、F2 blobs 和正常 centered 按完成标记续跑. score-only 幂等增加或替换 F2_centered.npz 中的 score/selected.
...
if (($# != 1)) || [[ "$1" != "5" ]]; then
    echo "[find1_train_shard_05][错误] 唯一合法参数是用户片号 5。" >&2
    exit 2
fi

# SHARD_NUMBER: int, 本实验获授权的 1-based 固定片号.
SHARD_NUMBER=5
...
verify_sha256() {
    ...
    if ! printf '%s  %s\n' "${expected_sha256}" "${input_path}" | sha256sum --check --status -; then
        echo "[find1_train_shard_05][错误] 冻结输入身份不一致：${input_path}" >&2
        exit 2
    fi
}
...
# centered 只前向来源体素数至少为 8 的 F2 blob. 当前 PDB 的 blob 数量超过上限时保留 _BLOB_EXCEED 标记并继续生成 centered.
