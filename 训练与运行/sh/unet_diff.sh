...
# unet_diff 正式训练入口; 使用实验密度与差分密度通道从头训练 density-only RAUNet64。
set -euo pipefail
...
if [[ "${resource_type}" != "a800" || "${nnodes}" != "1" || "${devices}" != "1" || "${cpu_count}" != "16" ]]; then
    echo "[unet_diff][错误] 本任务要求 resource=a800, nodes=1, gpus=1, cpus=16; 实际为 resource=${resource_type}, nodes=${nnodes}, gpus=${devices}, cpus=${cpu_count}。" >&2
    exit 2
fi
ddp_find_unused_parameters=false

experiment_group="AdaLigand_Stage1/unet_diff"
tag="unet_diff"
