...
用法:
  本地运行(不能检查缺失文件): python 此文件路径 --no-validate_files
  或服务器上: 
...
CSV_PATH = r"/storage/penghongen/EMDB_PDB_resolution_3.5.csv"

# str, 纯受体文件所在目录, 兼容嵌套与不嵌套模式
# RECEPTOR_CIF_DIR = "/storage/chenzhaoyang/cryo_em/CIF_3.5_atom"
RECEPTOR_CIF_DIR = "/storage/chenzhaoyang/cryo_em/result_split"

# bool, RECEPTOR_CIF_DIR是否嵌套(cryoatom时才嵌套)
# NEST_OR_NOT = False
NEST_OR_NOT = True
...
def expand_csv_rows(csv_path: str) -> List[Dict]:
    ...
    with open(csv_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            # str, 每个 EMDB 只取第一个有效 PDB ID
            first_pdb = next((p for p in pdb_list if p and p.lower() != "nan"), None)
            if first_pdb is None:
                continue

            # str, 标准化后的 EMDB ID（整个行共用一个）
            emdb_id_norm = normalize_emdb_id(raw_emdb)
            pdb_norm = normalize_pdb_id(first_pdb)
            rows.append({
...
def build_receptor_cif_path(receptor_dir: str, pdb_id: str, nest_or_not: bool) -> str:
    ...
    输入参数:
        - receptor_dir: str, 标量, 受体 CIF 根目录
        - pdb_id: str, 标量, 标准化后的 PDB ID
        - nest_or_not: bool, 标量, True 表示使用 {pdb_id_lower}/{pdb_id_lower}.cif 嵌套路径

    输出:
        - str, 标量, Linux 服务器侧 CIF 路径
    """
    pdb_id_lower = pdb_id.lower()
    if nest_or_not:
        return f"{receptor_dir}/{pdb_id_lower}/{pdb_id_lower}.cif".replace("\\", "/")
    return f"{receptor_dir}/{pdb_id}.cif".replace("\\", "/")


def build_emdb_map_path(emdb_dir: str, emdb_id: str) -> str:
    """
    生成 EMDB map 文件路径.

    输入参数:
        - emdb_dir: str, 标量, EMDB map 目录
        - emdb_id: str, 标量, 标准化后的 EMDB ID

    输出:
        - str, 标量, Linux 服务器侧 map 路径
    """
    return f"{emdb_dir}/{emdb_id}.map".replace("\\", "/")
...
def validate_input_files(
    ...
    输入参数:
        ...
        - receptor_dir: str, 标量, 受体 CIF 根目录
        - emdb_dir: str, 标量, EMDB map 目录
        - nest_or_not: bool, 标量, True 表示受体 CIF 使用嵌套路径

    输出:
        - tuple[list[dict], list[dict], list[dict]]: 有效条目、受体缺失条目、EMDB 缺失条目
    """
    validated_rows = []
    skipped_receptor = []
    skipped_emdb = []
    ...
    for row in rows:
        receptor_cif_path = build_receptor_cif_path(receptor_dir, row["pdb_id"], nest_or_not)
        emdb_map_path = build_emdb_map_path(emdb_dir, row["emdb_id"])

        if not os.path.exists(receptor_cif_path):
            skipped_receptor.append({**row, "missing_path": receptor_cif_path})
            continue
        if not os.path.exists(emdb_map_path):
            skipped_emdb.append({**row, "missing_path": emdb_map_path})
            continue

        validated_rows.append(row)

    return validated_rows, skipped_receptor, skipped_emdb
...
def main(
    ...
    print(f"服务器输出根目录:   {server_output_dir}")
    print(f"受体目录是否嵌套:   {nest_or_not}")
    print(f"是否检查输入文件:   {validate_files}")
    print(f"批次大小:           {batch_size}")
    print("=" * 70)

    # --- Step 1: 读取 CSV 并选择每个 EMDB 的第一个有效 PDB 条目 ---
    print("\n[Step 1] 读取 CSV 并选择每个 EMDB 的第一个有效 PDB 条目 ...")
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"CSV 文件不存在: {csv_path}")

    raw_rows = expand_csv_rows(csv_path)
    print(f"  条目数: {len(raw_rows)}")

    # --- Step 2: 检查服务器端输入文件是否存在 ---
    if validate_files:
        print("\n[Step 2] 检查受体 CIF 与 EMDB map 是否存在 ...")
        validated_rows, skipped_receptor, skipped_emdb = validate_input_files(
            rows=raw_rows,
            receptor_dir=receptor_cif_dir,
            emdb_dir=emdb_map_dir,
            nest_or_not=nest_or_not,
        )
        print(f"  有效条目数: {len(validated_rows)}")
    ...
    print(f"\n输出根目录: {output_dir}")
    print("\n上传至服务器后，在服务器上执行:")
    print(f"  bash {server_output_dir}/slurm/run_all.sh")
    print("=" * 70)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="生成 Chimera 命令文件：为 cryoatom 建模结构按真实分辨率生成模拟密度图"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="cryoatom 建模 CIF 根目录；实际按 {pdb_id_lower}/{pdb_id_lower}.cif 拼接（默认: RECEPTOR_CIF_DIR 常量）"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--server_output_dir", type=str, default=SERVER_OUTPUT_DIR,
        help="服务器上 .cmd/slurm/log 输出根目录（默认: SERVER_OUTPUT_DIR 常量）"
    )
    parser.add_argument(
        "--nest_or_not", action=argparse.BooleanOptionalAction, default=NEST_OR_NOT,
        help="受体 CIF 是否使用 {pdb_id_lower}/{pdb_id_lower}.cif 嵌套路径（默认: NEST_OR_NOT 常量）"
    )
    parser.add_argument(
        "--validate_files", action=argparse.BooleanOptionalAction, default=VALIDATE_FILES,
        help="生成 .cmd 前是否检查受体 CIF 与 EMDB map 是否存在（默认: VALIDATE_FILES 常量）"
    )
