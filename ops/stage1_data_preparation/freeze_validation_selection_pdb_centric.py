...
从 Pocket_Plus 根目录运行 ``python -m ops.stage1_data_preparation.freeze_validation_selection_pdb_centric``. 模块方式保证项目根目录参与 Python 包解析, 同时没有向脚本增加可变参数.
...
文件字段:
    ...
    - pdb_occurrence_foreground_box_cap: int32 标量; 单个 occurrence 每个 epoch 的 bias BOX 数量上限, 固定为 5.
"""
