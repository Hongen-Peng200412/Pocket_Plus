...
- 2026-08-31 21:58 +08:00，第十八次完整 validation 已结束。验证总损失降至 `0.2011843`，配体体素 PRAUC 创新高至 `0.5764135`，受体 PRAUC 创新高至 `0.6228836`；新的 `TOP_epoch_02_score_0.5764.ckpt` 与更新后的 `last.ckpt` 已生成，任务继续推进到 `trainer/global_step=20528`。`BEST.ckpt` 暂时保持第十六次的 `TOP_epoch_01_score_0.5507.ckpt`，符合已确认的回调时序；未发现 traceback、CUDA OOM、NCCL、DataLoader 失败或非有限值错误。
- 2026-09-01 02:45 +08:00，第十九次完整 validation 已结束。验证总损失继续降至 `0.1990959`，配体体素 PRAUC 为 `0.5634502`，受体 PRAUC 创新高至 `0.6248313`；新的 `TOP_epoch_02_score_0.5635.ckpt` 与更新后的 `last.ckpt` 已生成，任务继续推进到 `trainer/global_step=21626`。`BEST.ckpt` 已按既知的一次 validation 回调滞后刷新为第十八次的 `TOP_epoch_02_score_0.5764.ckpt`，未发现 traceback、CUDA OOM、NCCL、DataLoader 失败或非有限值错误。
- 2026-09-01 07:32 +08:00，第二十次完整 validation 已结束。验证总损失为 `0.2051169`，配体体素 PRAUC 为 `0.5622604`，受体 PRAUC 为 `0.6195352`；新的 `TOP_epoch_02_score_0.5623.ckpt` 与更新后的 `last.ckpt` 已生成，任务恢复训练并继续推进到 `trainer/global_step=22973`。`BEST.ckpt` 正确保持第十八次的 `TOP_epoch_02_score_0.5764.ckpt`，未发现 traceback、CUDA OOM、NCCL、DataLoader 失败或非有限值错误。
- 2026-09-01 12:15 +08:00，第二十一次完整 validation 已结束。验证总损失为 `0.2043146`，配体体素 PRAUC 为 `0.5645520`，受体 PRAUC 为 `0.6244339`；新的 `TOP_epoch_02_score_0.5646.ckpt` 与更新后的 `last.ckpt` 已生成，任务恢复训练并继续推进到 `trainer/global_step=23945`。`BEST.ckpt` 正确保持第十八次的 `TOP_epoch_02_score_0.5764.ckpt`，未发现 traceback、CUDA OOM、NCCL、DataLoader 失败或非有限值错误。
- 2026-09-01 16:44 +08:00，第二十二次完整 validation 已结束。验证总损失为 `0.1995346`，配体体素 PRAUC 为 `0.5633904`，受体 PRAUC 为 `0.6191931`；新的 `TOP_epoch_02_score_0.5634.ckpt` 与更新后的 `last.ckpt` 已生成，任务恢复训练并继续推进到 `trainer/global_step=25112`。连续四次验证未超过调度器记录的配体体素 PRAUC `0.5764135` 后，冻结配置中的 `ReduceLROnPlateau` 按 `factor=0.2`、`patience=3`、绝对改进阈值 `0.003` 将学习率从 `1e-4` 降至 `2e-5`。`last.ckpt` 中调度器状态为 `num_bad_epochs=0`，`LearningRateReductionStopper` 已记录第一次衰减；配置要求第二次实际衰减后正常停止训练。`BEST.ckpt` 保持第十八次的 `TOP_epoch_02_score_0.5764.ckpt`，未发现 traceback、CUDA OOM、NCCL、DataLoader 失败或非有限值错误。
- 2026-09-01 21:11 +08:00，第二十三次完整 validation 已结束。验证总损失创新低至 `0.1895426`，配体体素 PRAUC 创新高至 `0.5819030`，受体 PRAUC 创新高至 `0.6446361`；新的 `TOP_epoch_02_score_0.5819.ckpt` 与更新后的 `last.ckpt` 已生成，任务恢复训练并继续推进到 `trainer/global_step=26159`。配体体素 PRAUC 较调度器旧基准 `0.5764135` 提高约 `0.00549`，超过绝对改进阈值 `0.003`；`last.ckpt` 已保存新基准 `0.5819030`、`num_bad_epochs=0`、学习率 `2e-5` 和第一次实际衰减计数。`BEST.ckpt` 暂时保持第十八次的 `TOP_epoch_02_score_0.5764.ckpt`，符合已确认的一次 validation 回调滞后；未发现 traceback、CUDA OOM、NCCL、DataLoader 失败或非有限值错误。
- 2026-09-03 14:47 +08:00，Job `358384` 仍在 `hnode01` 使用 H100×1、CPU×32 训练。W&B run `9dsi7i9w` 已推进到 `trainer/global_step=36269`、学习率 `2e-5`、训练总损失约 `0.1894`；当时最近一次完整 validation 总损失为 `0.1875641`、配体体素 PRAUC 为 `0.5937543`、受体 PRAUC 为 `0.6552877`。`TOP_epoch_03_score_0.5938.ckpt` 与 `last.ckpt` 已于 11:16 写出，均为 499,678,098 bytes。
- 2026-09-03 15:45—15:46 +08:00，训练在第二次实际学习率衰减后按 `stop_after_lr_reductions=2` 正常停止。W&B 最终摘要为 epoch 3、`trainer/global_step=36349`、学习率 `2e-5`、验证总损失 `0.1856628805`、配体体素 PRAUC `0.5956817269`、受体 PRAUC `0.6590440273`；包装器记录训练总时长 540,795.94 秒，并明确写出“正式训练完成”和“第 1 次执行成功”。W&B 于 15:46:17 完成最终同步。
- 最终训练产物均位于 `/home/penghongen/Feedback/Pocket_Plus/logs/AdaLigand_Stage1_pdb_centric_2-unet_c1-mainchain/unet_c1_mainchain_pdb_centric_2____unet_c1_job358384_20260828T093205_a1_formal`。`checkpoints/BEST.ckpt`、`checkpoints/TOP_epoch_03_score_0.5957.ckpt` 与 `checkpoints/last.ckpt` 均为 499,678,098 bytes，SHA-256 均为 `8e8bd068ebbebeb8b49db3ddd40c6c485bad322e75f3308b39298b8cea7bec3a`；稳定别名已经追上最终最高分 TOP。
- 训练 launch 的 `launch.json` 与 `run_cmd.sh` SHA-256 分别为 `e891f5f13519f2c3e2ecc46d5af6938533ac385ce0ac6b528bf7695accb6e116` 和 `c365ce608826031abb21963f5fb51b1b90cea67cc7d83b40fa8e633e40e0240a`。运行目录中的 `config.yaml`、`train.yaml` 与 `src_snapshot/manifest.json` SHA-256 分别为 `0e56a76a8b3bafc0312972567c72835763c9ea732254101322410fefb6d3229d`、`3eb0e9810e34d522f5d421b82f321c09967b9750beab7eebf37b819564487d42` 和 `d3105c67e8da0343b95258d156a93a388323522ab4ffbc002bf7601820884a9a`。
- 训练成功后，同一 Job 的 `try_lock_358384` 被另一条已安排的推理工作消费。第二次执行的 release、launch 和 `run_cmd.sh` 分别为 `/home/penghongen/Feedback/Pocket_Plus/releases/Pocket_Plus_d57060538839/Pocket_Plus`、`/home/penghongen/Feedback/Pocket_Plus/launches/358384/unet_c1_job358384_20260903T161305_a2` 和 SHA-256 `67948adeff133468605e6bc1dda25921da4e37d9b0fed06cc36ea1252afdcac3`；命令为 `exec bash "${TASK_PROJECT_ROOT}/训练与运行/sh/infer/unet_c1_sampling_comparison.sh" pdb_centric_2`，其 calibration probability 输出根为 `/storage/penghongen/AdaLigand_stage1_inference/UNET/unet_c1_pdb_centric_v2/artifacts`。这段信息只用于说明 allocation 当前仍有工作，不把推理执行纳入本训练记录的控制范围。
- 2026-09-03 16:13 +08:00，`after_lock_358384` 仍存在，Slurm allocation 仍占用 hnode01 的 H100，且第二次执行的推理进程正在运行。用户最新明确要求保留该资源用于其他工作；该指令取代此前“训练结束后释放”的安排。未经新的明确授权，禁止删除 `after_lock_358384`、取消 Job `358384` 或以其他方式释放该 H100。

## 2026-09-02 采样方式二 unet_c1 正式训练收口

- Job `356953` 是采样方式二 `pdb-centric-1` 的 `unet_c1 mainchain` 正式训练，而不是采样方式三 Job `358384` 的续训。其 Slurm 资源为 `nvlink` partition、`h200g4` QOS、`gnode09`、1 张 A800 和 24 CPU；提交参数为 `pre_hold=0`、`after_hold=1`。Job 于 2026-08-26 11:09:47 +08:00 启动。
...
- 正式运行目录为 `/home/penghongen/Feedback/Pocket_Plus/logs/AdaLigand_Stage1_pdb_centric-unet_c1-mainchain/unet_c1_mainchain____unet_c1_job356953_20260826T110721_a1_formal`；W&B run 为 `pencounkdual-111/AdaLigand_Stage1/6uuzdbgs`。最终 `config.yaml` 的 SHA-256 为 `3156d81fca04b1ab897e519febf05864c5b9563aca614f1704d5f83e07c90347`。
- 最终配置确认 Dataset 为 `stage1_unet_c1`，输入只有 `exp_clipnorm_nopost` 一个实验密度通道；训练请求使用 `25/0.5/25`，验证读取 `/storage/penghongen/AdaLigand/Ori_Data/stage1_preparation_box_pool_3/box_pool/validation_selection_pdb_centric.npz`。该 V1 文件的 SHA-256 为 `546ebd3a1f07b230af42911b6740f466c6af289c8bff91a387c4eb8b1d69dd8e`，实测包含 150 个 PDB、3,750 个 bias BOX、3,750 个 context BOX 和 0 个 center BOX。训练划分的 13,717 个 PDB 每个产生 25 个 bias 与 25 个 context，因此每个 epoch 为 685,850 个训练 BOX。
- 训练制度为每卡 batch 8、全局 batch 48、梯度累积 6、24 个 `DataLoader` workers、学习率 `1e-4`、`max_epochs=70`、`val_per_epoch=12`、`warmup_ratio=0.005`、`patience=3` 和 `stop_after_lr_reductions=2`。结构输出头开启；配体区域、配体体素、蛋白主链、核酸主链和配体反距离五项损失权重分别为 `0.1/1.0/0.05/0.05/0.3`。
- 训练累计保存 20 个 TOP checkpoint。W&B 最终摘要记录 `trainer/global_step=23813`、epoch 1、验证总损失 `0.1979677`、配体体素 PRAUC `0.4891676` 和受体 PRAUC `0.5830685`；`last.ckpt` 的内部 `global_step=23814`，`LearningRateReductionStopper` 已记录两次实际衰减，优化器学习率为约 `4e-6`。两种 step 表示相邻的日志索引与 checkpoint 内部计数，不构成训练状态冲突。
- 最佳配体体素 PRAUC 为 `0.4918`，对应 `checkpoints/TOP_epoch_01_score_0.4918.ckpt`。`BEST.ckpt` 与该 TOP 文件字节一致，SHA-256 均为 `0bb6bd5231c638c13dee5144bfb361b6bd20f5268bd10337a040c8486efa9b5a`；`last.ckpt` 的 SHA-256 为 `ce1408095c49095ea31bd78e78dbdd6ba441e6ae48ea09ecc485f1de5a34d1f0`。
- 标准输出明确记录“检测到实际 LR 衰减 2 次”“正式训练完成”“第 1 次执行成功”，训练包装器于 2026-09-01 06:48:18 +08:00 创建 `/home/penghongen/Feedback/Pocket_Plus/allocations/try_lock_356953`。严格错误扫描没有发现 traceback、CUDA OOM、CUDA error、NCCL 失败、DataLoader worker 异常、非有限损失、段错误或运行时异常。
- 2026-09-02 14:09 +08:00，Slurm 仍显示 Job `356953` 为 `RUNNING`，这是 `after_hold=1` 的预期状态。`gnode09` 上没有训练进程，A800 抽样显存为 2/81,920 MiB、GPU 利用率为 0%，也没有计算进程；`after_lock_356953` 与 `try_lock_356953` 均未被触碰。
- 前因与后果：采样方式二来自提交 `f806f12007e2cef521f71588103edf17318e8b4a`，目标是在每个 PDB 固定 25 个 bias BOX 并尽可能均分给 occurrence。采样方式三实现提交 `b4529f574e8d1728183a4e72f64c5b80b20a0215` 于方式二训练期间产生，随后以独立 Job `358384` 启动；两者是并列实验分支，不是续训关系。方式二训练产物已经可用于推理，但截至本次核验，尚未发现 Job `356953` 的 complete-map 推理与统一评估产物；三种采样方式仍须在同一 complete-map 数据和指标契约下比较，不能直接比较各自 BOX validation PRAUC。

## 接续位置

后续训练或推理适配不得重复构造 split 或 BOX pool，也不得重新写入迁移后的 NPZ。采样方式二与方式三的活动科学说明位于 `talk/global/global_8.26.md`；Job `356953` 的训练收口和后续 complete-map 边界记录在 `CLAUDE/memory/handoffs/2026-09-02-stage1-pdb-centric-1-unet-c1-complete.md`，Job `358384` 的当前状态、恢复边界和后续检查项记录在 `CLAUDE/memory/handoffs/2026-08-28-stage1-pdb-centric-v2-unet-c1-running.md`。
