...
本文件只生成服务器路径 ``/storage/penghongen/AdaLigand/Ori_Data/stage1_preparation_box_pool_3/box_pool/validation_selection_pdb_centric_v2.npz``. 输入路径, 输出路径, 随机种子和三个采样参数均在模块常量中固定, 不接受命令行参数.

从 Pocket_Plus 根目录运行 ``python -m ops.stage1_data_preparation.freeze_validation_selection_pdb_centric_2``. 模块方式保证项目根目录参与 Python 包解析, 同时没有向脚本增加可变参数. 脚本使用 validation manifest 的全部 200 个 PDB, 并以 seed 3407 冻结 epoch 0 的 bias 与 context 候选.
...
形状符号:
    - P: ``validation_pdb_id`` 保存的冻结 validation PDB 数量; 正式值为 200.
    - N_bias: bias 请求数量.
...
文件字段:
    ...
    - pdb_foreground_box_num: int32 标量; 每个 PDB 的目标 bias BOX 数量, 固定为 50.
    - pdb_foreground_fraction_target: float64 标量; 目标 bias 比例, 固定为 25/33 的十进制值, 使每个 PDB 抽取 16 个 context BOX.
    - pdb_occurrence_foreground_box_cap: int32 标量; 单个 occurrence 每个 epoch 的 bias BOX 数量上限, 固定为 1.
"""
...
OUTPUT_PATH = BOX_POOL_ROOT / "validation_selection_pdb_centric_v2.npz"
# int, 与正式 Dataset 配置一致的 epoch 0 请求随机种子。
REQUEST_SEED = 3407
# int, 每个 PDB 在 cap 允许时最多抽取的 bias BOX 数量。
PDB_FOREGROUND_BOX_NUM = 50
# float, 目标公式中的 bias 比例 25/33；当前请求实现据此为每个 PDB 抽取 16 个 context BOX。
PDB_FOREGROUND_FRACTION_TARGET = 0.7575757575757576
# int, 单个 occurrence 在一个 epoch 内最多抽取的 bias BOX 数量。
PDB_OCCURRENCE_FOREGROUND_BOX_CAP = 1


def main() -> None:
    """冻结正式 validation 的 epoch 0 请求并原子替换 V2 目标 NPZ.
    ...
    状态变化:
        - 原子写入 ``OUTPUT_PATH``; 重复运行会用同一组确定性字段替换已有 V2 文件, 不读取、删除或覆盖 V1 ``validation_selection_pdb_centric.npz``, 也不修改原 ``validation_selection.npz``, 单 PDB pool, manifest, config, summary 或 ``_COMPLETE``.

    输出文件:
        - ``OUTPUT_PATH``: 未压缩 NPZ; 正式保存 P=200 个 PDB, N_bias=3,305 个 bias 请求, N_context=3,200 个 context 请求和 0 个 center 请求.
        ...
        - ``pdb_foreground_box_num``: int32 标量; 每个 PDB 的目标 bias BOX 数量, 值为 50.
        - ``pdb_foreground_fraction_target``: float64 标量; 目标 bias 比例, 值为 25/33 的十进制值, 对应每个 PDB 抽取 16 个 context BOX.
        - ``pdb_occurrence_foreground_box_cap``: int32 标量; 单个 occurrence 每个 epoch 的 bias BOX 数量上限, 值为 1.
    """
    ...
    # tuple[ResolvedStage1Crop, ...], 按 validation manifest PDB 顺序排列的全部 200 个 PDB 的 bias 与 context 请求.
    ...
    # dict[str, np.ndarray], validation_selection_pdb_centric_v2.npz 的完整字段集合.
    arrays = {
