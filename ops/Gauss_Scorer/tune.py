...
from src.inference.Gauss_Scorer import compute_f1_centered_gauss_terms
...
def _load_grid(path: str | Path) -> tuple[dict[str, Any], str]:
    ...
    for field in (
        ...
        if not isinstance(values, list) or not 2 <= len(values) <= 4:
            raise ValueError(f"{field} 必须包含 2 至 4 个候选值")
...
def enumerate_configs(grid: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    configs: list[dict[str, Any]] = [{"config_index": 0, "kind": "baseline"}]
...
def _load_pdb_facts(
    *,
    ...
    centered = load_npz_strict(paths.centered_npz("F1_centered"))
    ...
    for tau in tau_values:
        positive, negative = compute_f1_centered_gauss_terms(
        ...
        if candidate_rows is None:
            ...
            raise ValueError(f"{pdb_id}: 不同 tau 的 F1 节点集合不一致")
...
def _evaluate_shard(arguments: argparse.Namespace) -> int:
    ...
    grid, grid_sha256 = _load_grid(arguments.grid_json)
...
def _merge(arguments: argparse.Namespace) -> int:
    ...
    for path in part_paths:
        ...
        if payload["grid_sha256"] != grid_sha256:
            raise ValueError(f"{path}: grid_sha256 不一致")
        current_identity = (
    ...
    atomic_write_json(
        arguments.calibration_json,
        {
            ...
            "baseline_metrics": ordered[0]["metrics"],
