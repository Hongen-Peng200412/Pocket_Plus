...
# 本脚本只在全部参数扫描分片成功后运行；它验证网格结果的精确覆盖并冻结唯一 calibration.json。
...
output_root="${GAUSS_OUTPUT_ROOT:-/storage/penghongen/AdaLigand_stage1_inference/Find_0-CPC1-ligand_PRAUC_0.675477/artifacts}"
if [[ -n "${GAUSS_CALIBRATION_JSON:-}" ]]; then
    calibration_json="${GAUSS_CALIBRATION_JSON}"
elif [[ "${centered_role}" == "F1_centered" ]]; then
    calibration_json="${output_root}/Find_0/gauss_scorer/calibration.json"
else
    calibration_json="${output_root}/Find_0/gauss_scorer/${centered_role}/calibration.json"
fi
