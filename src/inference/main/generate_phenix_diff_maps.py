...
from src.inference.main.build_baseline_cache import derive_phenix_map_path
...
def main(argv: list[str] | None = None) -> None:
    ...
    # set[tuple[str, str]], 已处理的 (system, sample_name), 用于去重
    processed: set[tuple[str, str]] = set()
    for system in args.systems:
        for sample in all_samples:
            # str, 样本名
            sample_name = str(sample["sample_name"])
            if (system, sample_name) in processed:
                continue
            processed.add((system, sample_name))
            ...
            try:
                ...
                failures.append({"system": system, "sample_name": sample_name, "error": str(exc)})
    ...
    summary_path = os.path.join(str(args.phenix_output_root), "generate_summary.json")
    with open(summary_path, "w", encoding="utf-8") as handle:
        json.dump({"generated": run_rows, "failures": failures}, handle, ensure_ascii=False, indent=2)
    print(f"[generate_phenix_diff_maps] done. 成功 {len(run_rows)}, 失败 {len(failures)}, 摘要: {summary_path}")
