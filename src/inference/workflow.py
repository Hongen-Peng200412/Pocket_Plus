...
本模块直接连接单 PDB 概率, CPU 连通区域, centered GPU 前向, 评分和评估.
命令行解析与 checkpoint/Dataset 构造留在 ``cli.py``.
"""
...
def evaluate_and_publish_role(
    ...
    匹配身份以及 top-K 获胜身份. 数据划分目录另存逐 PDB JSONL 和 micro/PDB
    等权 macro 指标; 缺少任一正式文件或完成标记时直接失败. 返回值是与
    `<centered_role>.metrics.json` 相同的跨 PDB 指标映射. 本入口生成:

    - `<pdb>/evaluation/<centered_role>.npz`, 保存阈值轴, top-K 轴, occurrence
      身份, 候选身份/分数/选择, 交集与尺寸, 语义计数, 覆盖掩码, 一对一匹配
      offsets/身份值表和 top-K 获胜身份.
    - `<split>/evaluation/<centered_role>.jsonl`, 每个 PDB 一条指标记录.
    - `<split>/evaluation/<centered_role>.metrics.json`, micro 与 PDB 等权 macro
      汇总. 评估文件不是生产角色, 不建立 `_COMPLETE`.
    """
    ...
    publish_stage1_jsonl(
        evaluation_root / f"{centered_role}.jsonl", jsonl_rows
    )
    publish_stage1_json(
...
def run_calibration_workflow(
    ...
    """生成 calibration 产物并最后发布冻结身份.

    ``dataset`` 必须能物化 ``pdb_ids`` 的 full-map 与 centered 请求;
    ``config`` 必须显式提供窗口、角色、三阶段校准、评估和有界队列参数;
    ``artifact_identity`` 绑定 producer, checkpoint, resolved config, 推理配置,
    语义分母和模型代码来源. 每次调用先撤销旧 calibration `_COMPLETE` 并重新
    计算所有 PDB 概率, 不复用缺少身份字段的旧概率完成标记.
    ...
    返回值与 `calibration/stage1_v3.json` 相同, 包含 `identity`, `semantic` 和
    F1/F3 `roles`. 另生成 `semantic_threshold_scan.npz`,
    `stage1_v3.metrics.json` 以及逐 PDB 五类科学文件、性能 JSON 和评估文件.
    最后的 `calibration/_COMPLETE` 精确保存 `identity` 和
    ...
    semantic = {
        name: value
        for name, value in semantic_with_scan.items()
        if name != "scan"
    }
    ...
    publish_stage1_json(
        ...
        {"identity": dict(artifact_identity), "roles": role_metrics},
    )
    publish_stage1_json(
        calibration_root / "stage1_v3.json", calibration_payload
    )
    publish_stage1_json(
        calibration_root / "_COMPLETE",
        {"identity": dict(artifact_identity), "result_scope": "calibration_fitted"},
    )
...
def run_frozen_workflow(
    ...
    ``calibration_complete`` 来自同目录 `_COMPLETE`. 两者身份必须与当前
    ``artifact_identity`` 完全相同, 且完成标记结果范围必须是
    `calibration_fitted`. ``pdb_ids`` 由 CLI 保证小写且无重复.

    calibration 身份必须与当前 producer, checkpoint, 训练配置, 推理配置和
    语义阈值分母完全相同. 每个完整图返回后, 概率压缩及 F1/F3 CPU 连通区域
    ...
    if dict(calibration_payload["identity"]) != dict(artifact_identity):
        raise ValueError("calibration 身份与当前 checkpoint 或配置不一致.")
    if dict(calibration_complete.get("identity", {})) != dict(artifact_identity):
        raise ValueError("calibration 完成标记身份与当前 checkpoint 或配置不一致.")
    if calibration_complete.get("result_scope") != "calibration_fitted":
        raise ValueError("calibration 完成标记缺少 calibration_fitted 结果范围.")
    pending_probability: deque[
        tuple[Future[None] | None, tuple[Future[dict[str, np.ndarray]], ...]]
    ] = deque()
    with ThreadPoolExecutor(
        max_workers=int(config.blob_workers),
        thread_name_prefix="stage1-blobs",
        ...
        for pdb_id in pdb_ids:
            ...
            probability_future: Future[None] | None = None
            ...
            probability_map = probability["probability_map"]
            blob_rows = []
            for role_name, role_config in config.roles.items():
            ...
            if len(pending_probability) >= int(config.pending_probability_pdbs):
                ...
                if oldest_probability is not None:
