...
.json 形如 list[dict], 每项含 {"cif_path": ..., "map_path": ...} 即实际的路径对, 这是 Pocket\src\inference\run.py 里面  def _run_raw_batch_mode 直接接受的.
...
_POCKET_ROOT = _CURRENT_DIR.parent.parent.parent
if str(_POCKET_ROOT) not in sys.path:
    sys.path.insert(0, str(_POCKET_ROOT))
...
def get_nucleic_ratio(file_path: str) -> float:
    ...
    - nucleic_ratio: float , 标量, 计算出的比例值（非HETATM核酸数量 / 非HETATM氨基酸数量）。
      如果没有氨基酸但有核酸，则返回 inf；只要核酸都为空，都是 0.0。
    """
    # str, 抽取到的文件扩展名后缀
...
def generate_sample_json(
    ...
    # 输入参数:
        ...
        - valid_json_path: str, 标量, 必选的范围界定json文件(也提供emdb-pdb映射)
    ...
    # 输出:
        - output_json_path: str, 标量, 最后将用于生成保存完整结果表的带有后缀文件名字的全绝对路径值
    """
    ...
    # dict, str:str, 表示 这个结构文件的小写id:对应全路径（用于提取模型输入特征的结构，即 pdb_folder 中的文件）
    pdb_dict = {}
    if os.path.exists(pdb_folder):
        for p_file in os.listdir(pdb_folder):
            p_ext = os.path.splitext(p_file)[-1].lower()
            if p_ext in [".cif", ".pdb", ".mmcif"]:
                p_base = os.path.splitext(p_file)[0].lower()
                if p_base not in pdb_dict:
                    pdb_dict[p_base] = os.path.join(pdb_folder, p_file)

    # dict, str:str, 可选，真实结构文件的小写id:对应全路径（用于提取 GT 标签，即 pdb_gt_folder 中的文件）
    # 若 pdb_gt_folder 为 None 则为空字典，不影响后续逻辑
    pdb_gt_dict = {}
    if pdb_gt_folder is not None and os.path.exists(pdb_gt_folder):
        for gt_file in os.listdir(pdb_gt_folder):
            gt_ext = os.path.splitext(gt_file)[-1].lower()
            if gt_ext in [".cif", ".pdb", ".mmcif"]:
                gt_base = os.path.splitext(gt_file)[0].lower()
                if gt_base not in pdb_gt_dict:
                    pdb_gt_dict[gt_base] = os.path.join(pdb_gt_folder, gt_file)
    ...
    for pair in candidate_pairs:
        ...
        # 若提供了真实结构目录 pdb_gt_folder，则尝试查找同名真实结构文件，写入 cif_gt_path
        if cif_gt_path is not None and os.path.exists(cif_gt_path):
            # 注意：将 cif_gt_path 放在 cif_path 之前，JSON 中阅读更直观
            acc_dict = {
...
# NOTE：用来调参的.json, 对核酸比例无要求, 先跑这个找最优参数, 然后再跑下面的(基准.json为"/home/penghongen/My_Project/Data/split/3.5_cc_qscore_v0/val.json")
"""
(vnegnn) [penghongen@gnode11 ~]$ python /home/penghongen/My_Project/Pocket/src/inference/utils/yield_json_from_raw_sample.py
---------- 开始生成样本 JSON 文件 ----------
提取到的初始候选配对总数: 268
---------- 扫描完成，统计信息 ----------
  - 初始检查候选配对总数: 268
  - 实际执行文件和比例探测运算的配对次数: 30
  - 缺失结构或密度图文件配对数: 0
  - 因核酸比例不合格而过滤抛弃数: 0
  - 最终保留且符合要求的有效配对数: 30
✅ json 生成完毕! 输出在: /home/penghongen/My_Project/Pocket/src/inference/utils/for_v0_search.json
"""


# NOTE: 纯蛋白测试样本(基准.json为"/home/penghongen/My_Project/Data/split/3.5_cc_qscore_v0/test.json")
"""
(vnegnn) [penghongen@gnode11 ~]$ python /home/penghongen/My_Project/Pocket/src/inference/utils/yield_json_from_raw_sample.py
---------- 开始生成样本 JSON 文件 ----------
提取到的初始候选配对总数: 268
---------- 扫描完成，统计信息 ----------
  ...
  - 因核酸比例不合格而过滤抛弃数: 10
  - 最终保留且符合要求的有效配对数: 50
✅ json 生成完毕! 输出在: /home/penghongen/My_Project/Pocket/src/inference/utils/for_v0_protein_eval.json
"""
...
---------- 扫描完成，统计信息 ----------
  ...
  - 因核酸比例不合格而过滤抛弃数: 255
  - 最终保留且符合要求的有效配对数: 13
✅ json 生成完毕! 输出在: /home/penghongen/My_Project/Pocket/src/inference/utils/for_v0_nucleic_eval.json
"""
...
---------- 扫描完成，统计信息 ----------
  - 初始检查候选配对总数: 11037
  - 实际执行文件和比例探测运算的配对次数: 11037
  - 缺失结构或密度图文件配对数: 11031
  - 因核酸比例不合格而过滤抛弃数: 0
  - 最终保留且符合要求的有效配对数: 6
✅ json 生成完毕! 输出在: /home/penghongen/My_Project/Pocket/src/inference/utils/for_cryoAtom_eval_easy.json
"""


# NOTE: cryoAtom的困难测试(不要求cc<3.5A)  !! 目前映射关系不全，无法真正完成！！
"""
