...
数据划分 `.jsonl` 每个已评估 PDB 一条 `pdb_id` 加指标映射；`.metrics.json` 保存同一公式的跨 PDB 汇总。两者与逐 PDB NPZ 使用同一个显式 `evaluation-name`。固定键是 `pdb_count`、`semantic_tp`、`semantic_fp`、`semantic_fn`、`semantic_micro_precision`、`semantic_micro_recall`、`semantic_micro_f1`、`semantic_micro_f2`、`semantic_macro_precision`、`semantic_macro_recall`、`semantic_macro_f1`、`semantic_macro_f2`、`semantic_micro_prauc`、`semantic_macro_prauc` 与 `topk_eligible_pdb_count`。每个覆盖阈值标签 `{t}` 生成 `coverage_micro_precision_{t}`、`coverage_micro_recall_{t}`、`coverage_micro_f1_{t}`、`coverage_micro_f2_{t}`、`coverage_macro_precision_{t}`、`coverage_macro_recall_{t}`、`coverage_macro_f1_{t}`、`coverage_macro_f2_{t}`、`coverage_micro_prauc_{t}`、`coverage_macro_prauc_{t}`，以及同样十个 `one_to_one_*_{t}` 键。每个 top-K 值 `{k}` 与阈值标签 `{t}` 生成 `top{k}_success_count_{t}` 和 `top{k}_success_ratio_{t}`。micro precision/recall 先跨 PDB 汇总命中数与对应预测或真实实体数再相除；macro precision/recall 先逐 PDB 计算再作算术平均，与 macro F1/F2 分别平均逐 PDB F-beta 的口径一致。阈值标签把小数点改为 `p`，例如 0.3 写成 `0p3`；任一分母为零时保存 0.0。
