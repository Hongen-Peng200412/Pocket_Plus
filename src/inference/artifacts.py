...
class Stage1ArtifactPaths:
    ...
    def calibration_root(self) -> Path:
        """返回当前 producer 的冻结阈值、评分参数和校准报告目录."""
    ...
    def evaluation(self, role: str, suffix: str) -> Path:
        """返回当前 PDB 的逐候选评估 NPZ 或 JSONL 路径."""

        return self.pdb_root / "evaluation" / f"{role}.{suffix}"
...
def load_stage1_npz(
    ...
    """读取一个 Stage1 NPZ 并核对调用者真正消费的字段.

    输入参数:
        - path: str | Path, 目标 NPZ 文件.
        - required_fields: Sequence[str], 当前调用位置要求存在的字段名; 允许文件包含其他正式字段.

    返回值:
        - arrays: dict[str,np.ndarray], 文件中的全部数组; 不允许 object dtype, 以保证 `allow_pickle=False` 可冷读.
    """
...
def publish_stage1_artifact(
    ...
    发布边界:
        - 临时 NPZ 位于最终文件同一目录, 写完后以 `allow_pickle=False` 重读字段名、dtype 和形状, 再调用 `os.replace`.
    ...
    try:
        ...
        reread = load_stage1_npz(temporary, tuple(values))
        mismatched = [
            name
            for name, value in values.items()
            if reread[name].dtype != value.dtype or reread[name].shape != value.shape
        ]
        if mismatched:
            raise ValueError(f"{temporary}: 重读字段 dtype 或形状改变 {mismatched}.")
        os.replace(temporary, target)
        if complete_path is not None:
            marker = Path(complete_path)
            marker.parent.mkdir(parents=True, exist_ok=True)
            marker_handle, marker_name = tempfile.mkstemp(
                prefix="._COMPLETE.",
                suffix=".tmp",
                dir=marker.parent,
            )
