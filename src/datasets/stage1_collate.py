...
每个 BOX 的稠密体素字段沿批次维堆叠。不同 BOX 的受体原子数量不同，原子字段
不补齐到相同长度，而是按样本顺序拼接成 ``(N_atom_total, ...)`` 数组；
``atom_offsets``、``atom_counts`` 和 ``atom_batch_index`` 共同保存每个 BOX
在该拼接数组中的范围。
"""
...
class Stage1BatchCollator:
    ...
    def __call__(self, samples: Sequence[dict[str, Any]]) -> dict[str, Any]:
        """
        把同一模型生产器的单样本序列整理成固定体素网格与变长原子批次。
