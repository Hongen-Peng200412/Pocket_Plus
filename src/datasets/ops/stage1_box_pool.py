...
def _atomic_save_npz(path: Path, **arrays: np.ndarray) -> None:
    ...
    输入参数:
        ...
        - arrays: np.ndarray 关键字映射，每个键成为一个 NPZ 字段
...
def _load_occurrence_masks(path: Path, expected_shape_zyx: Sequence[int]) -> dict[int, np.ndarray]:
    ...
    输入参数:
        ...
        - expected_shape_zyx: Sequence[int], (3,), 对应 exp 完整图的 ZYX shape

    输出:
        - masks: dict[int,np.ndarray], occurrence_id 到 (K_occ,3) int32 ZYX voxel indices 的映射
    """
...
def generate_context_starts(
    ...
    输出:
        - context_starts_zyx: np.ndarray, (N_context,3), int32，合法的完整图离散 ZYX voxel-index context BOX corner 起点；N_context 不超过 `target_count`
...
def build_occurrence_pool_rows(
    ...
    # np.ndarray[int32], (N_occ,), 按 occurrence_id 升序固定各 pool 数组的第 0 轴。
...
def build_pdb_box_pool(
    ...
    """
    从一份 A-G PDB 三件套构造 center、bias 和 context 索引池。

    输入参数:
        - data_root: str | Path, A-G 正式数据根目录
        - pdb_id: str, 当前 PDB identity
        - split_name: str, 当前 split 名称；参与 PDB seed 派生
...
def freeze_validation_selection(
    ...
    # 下列平行列表共同编码 selection：pdb_index 指向 validation_pdb_id，
    # occurrence_id/candidate_index 再索引相应 PDB pool；同类列表长度必须一致。
