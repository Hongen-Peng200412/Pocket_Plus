...
def freeze_stage1_splits(
    ...
    参数:
        keep_list_path: Stage G ``keep_list.jsonl``，其中同一 PDB 可含多条 occurrence 行。
        data_root: A—G 正式数据根目录，用于读取 ``density/{pdb_id}/exp.npz`` 的
            ``canonical_shape_zyx``。
        output_root: 本次 split run 的独立输出目录。
        seed: 冻结排名 seed，正式值为 ``3407``。
        validation_pdb_count: validation 的 PDB 数；正式值为 ``300``。
        calibration_pdb_count: calibration 的 PDB 数；正式值为 ``100``。

    返回:
        与 ``summary.json`` 相同的 PDB/row 计数。train PDB 数严格为
