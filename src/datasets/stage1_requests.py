...
class Stage1TrainingRequestSet:
    ...
    def __init__(
        ...
        # tuple[_PdbPool, ...]，每个元素保存一个 PDB 的 occurrence 编号及 center、bias、context ZYX 起点数组。
        self._pools = tuple(
    ...
    def _build_epoch_requests(self, epoch: int) -> tuple[ResolvedStage1Crop, ...]:
        ...
        # 同一 seed 和训练周期复用同一随机流；PDB 顺序、occurrence 和 BOX 候选均由该随机流确定。
        ...
        for pool_index in rng.permutation(len(self._pools)).tolist():
            ...
            # int64, (capped_count,)，数值索引当前 PDB 的 occurrence_id、center_start_zyx 和 bias_start_zyx 第一维。
            capped_rows = rng.permutation(occurrence_count)[:capped_count]
            selected_count = math.ceil(capped_count * self.occurrence_ratio)
            # int64, (selected_count,)，当前训练周期真正展开为 BOX 请求的 occurrence 位置。
