...
def average_precision_full_grid(
    ...
    输入参数:
        - probability_map: np.ndarray, (D,H,W), 完整图 ZYX voxel grid 上的连续概率；Find hardmask voxel 仍在区域内
        - gt_union_mask: np.ndarray, (D,H,W), 完整图 ZYX voxel grid 上的 occurrence ligand-area 并集标签

    输出:
        - average_precision: float | None, 与 sklearn `average_precision_score` 等价；
          当前 PDB 没有 GT 正类时返回 None，供 macro 聚合跳过并报告有效数
    """
    # np.ndarray[float64], (V,), 按 C-order 展平的完整图连续概率
    score = np.asarray(probability_map, dtype=np.float64).reshape(-1)
    # np.ndarray[bool], (V,), 与 score 同 voxel 顺序的 ligand union target
    ...
    if positive_count == 0:
        return None
    # np.ndarray[int64], (V,), 概率降序且同分稳定的 voxel 排名
    ...
    # np.ndarray[int64], (N_unique_score,), 每个不同 score 分组的末行
    boundaries = np.r_[
...
def semantic_dice(
    ...
    输入参数:
        - probability_map: np.ndarray, (D,H,W), 完整图 ZYX voxel grid 上的连续概率
        - gt_union_mask: np.ndarray, (D,H,W), 完整图 ZYX voxel grid 上的 occurrence ligand-area 并集
