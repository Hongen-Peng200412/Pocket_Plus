"""Candidate-Conditioned Lineage Network（CCLN）的首版实现。"""
...
def probability_features(probability: torch.Tensor, epsilon: float) -> torch.Tensor:
    """
    构造概率本身与稳定 logit 两维 token value 特征。

    输入参数:
        - probability: torch.Tensor, (N,), 已 sigmoid 的实体概率
        - epsilon: float, clip 到 [epsilon,1-epsilon] 的数值稳定常数

    输出:
        - features: torch.Tensor, (N,2)，依次为 p 与 log(p/(1-p))
    """
...
class CandidateConditionedLineageNetwork(nn.Module):
    ...
    输入参数:
        ...
        - v_output_dim: int, V5+D 输出通道；正式为 48
        - v_grid_source_order/v_grid_source_dims: 四张原生 V grid 来源及通道
        - v_meta_dim: int, V 基础 meta 通道；当前为概率二元组 2
        - v_fusion_projected_dim/v_fusion_gate_hidden_dim: V 多尺度 residual_swiglu 参数
        - v_correction_gate_hidden_dim: int, m/c correction gate hidden dim
        - use_v_multiscale/use_density_context: bool, V5 与 D 分支开关
    ...
    def __init__(
        ...
        v_output_dim: int,
        v_grid_source_order: Sequence[str],
        v_grid_source_dims: Mapping[str, int],
        v_meta_dim: int,
        v_fusion_projected_dim: int,
        v_fusion_gate_hidden_dim: int,
        v_correction_gate_hidden_dim: int,
        use_v_multiscale: bool,
        ...
        self.v_fusion = V5DensityFusion(
            grid_source_order=v_grid_source_order,
            grid_source_dims=v_grid_source_dims,
            meta_dim=int(v_meta_dim),
            output_dim=int(v_output_dim),
            fusion_projected_dim=int(v_fusion_projected_dim),
            fusion_gate_hidden_dim=int(v_fusion_gate_hidden_dim),
            correction_gate_hidden_dim=int(v_correction_gate_hidden_dim),
            use_multiscale=bool(use_v_multiscale),
    ...
    def _relative_bias_input(
        ...
        输入参数:
            - token_coord_world: torch.Tensor, (N_token,3)，token 世界 XYZ Å
            - candidate_center_world: torch.Tensor, (3,)，当前候选锚点
            - oldest_center_world: torch.Tensor, (3,)，CLG oldest 模态锚点
            - box_center_world: torch.Tensor, (3,)，当前 BOX 中心
            - box_half_size_world: torch.Tensor, (3,)，当前 BOX 各轴物理半边长 s
            - probability: torch.Tensor, (N_token,)，token 概率

        输出:
            - relative: torch.Tensor, (N_token,10)，三组 XYZ 相对坐标与 p
        """
    ...
    def _build_tokens(self, sample: Mapping[str, Any]) -> dict[str, torch.Tensor]:
        ...
        v_value = self.v_fusion(
            ...
            native_grids=sample["V_native_grids"] if self.v_fusion.use_multiscale else {},
            ...
            box_shape_zyx=sample["box_shape_zyx"].reshape(1, 3),
    ...
    def forward(self, sample: Mapping[str, Any]) -> dict[str, torch.Tensor]:
        ...
        输入参数:
            - sample: Mapping[str, Any]，至少包含:
                ...
                - candidate_centroid_world: torch.Tensor, (N_candidate,3)
                - candidate_voxel_offsets/candidate_voxel_index: candidate→V membership
                - tree_relative_feature: torch.Tensor, (N_candidate,N_candidate,8)
                - V_sources/V_native_grids/V_probability/V_coord_world/V_index_local_zyx/V_batch_index
                - box_center_world/box_half_size_world/box_shape_zyx
