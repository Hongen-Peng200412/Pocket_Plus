...
class DensityMUNetLite(nn.Module):
    ...
    输入参数:
        - input_shape_zyx: Sequence[int], 正式为 (80,80,80)；测试可用其它能被 8 整除的三轴 shape
    ...
    前向输入:
        - density_input: torch.Tensor, (B,1,D,H,W)，现场裁剪并构造的 exp_clipnorm_nopost

    前向输出:
        - density_feature: torch.Tensor, (B,32,D,H,W)，只供实际 V voxel gather，不落盘 dense grid
    """
    ...
    def _run_bottleneck_transformer(self, feature: torch.Tensor) -> torch.Tensor:
        ...
        输入参数:
            - feature: torch.Tensor, (B,C,D_b,H_b,W_b)，正式为 (B,128,10,10,10)
    ...
    def forward(self, density_input: torch.Tensor) -> torch.Tensor:
        ...
        输入参数:
            - density_input: torch.Tensor, (B,1,D,H,W)，exp_clipnorm_nopost

        输出:
            - density_feature: torch.Tensor, (B,32,D,H,W)，full-resolution 密度上下文
        """
