...
class _NodeCapExceeded(RuntimeError):
    """表示一个原子加入会使当前 CLG candidate 总数超过上限。"""
...
class _Attempt:
    ...
    def add_atomic(self, nodes: Sequence[ComponentNode]) -> bool:
        """全部节点 active/eligible 才原子加入；超过 cap 抛弃整个尝试。"""
...
def _expand_toward_higher_thresholds(
    ...
    """沿 direct children 扩展；unary 不耗预算，多路 split 原子加入全部 children。"""
...
def _expand_toward_lower_thresholds(
    ...
    """沿 direct parent 扩展；merge 原子加入 parent 与 sisters，并扩展新 sisters。"""
    current = seed
