...
class ProcessingError(RuntimeError):
    """
    Unified processing exception.

    Carries a normalized error_type so callers can branch consistently.
    """

    def __init__(self, error_type: str, error_detail: str, logged: bool = False):
...
def return_error_and_raise(
    ...
    """
    Log an error entry and raise a unified ProcessingError.
    """
    return_error_info(
        file_path=file_path,
        line=line,
    ...
    raise ProcessingError(error_type=error_type, error_detail=error_detail, logged=True)
...
class ErrorType:
    ...
    INVALID_CONFIGURATION = "INVALID_CONFIGURATION" # 非法参数/配置
    INVALID_LOCAL_FRAME = "INVALID_LOCAL_FRAME" # 局部坐标系无法构建
