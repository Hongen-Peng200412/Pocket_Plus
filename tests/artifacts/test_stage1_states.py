...
def test_formal_component_producer_uses_unique_threshold_layers_and_publishes(tmp_path: Path) -> None:
    ...
    def centered_adapter(output, batch):
        del output, batch
        return {
            ...
            "voxel_final_grid": np.broadcast_to(
                np.zeros((48, 1, 1, 1), dtype=np.float16), (48, *shape)
            ),
            **fixed_grids,
        }
    ...
    centered_producers = make_f1_clg_centered_role_producers(
        ...
        batch_builder_provider=lambda current_task: lambda request: {"request": request},
        output_adapter=centered_adapter,
    )
