...
def _write_unet_pdb(
    ...
    split: str,
    pdb_id: str,
    model_name: str = "unet_c1",
) -> None:
    """写出一个字段完整、数值极小的 CLG_centered 测试 PDB。"""
    pdb_root = root / model_name / split / pdb_id
    (pdb_root / "status" / "CLG_centered").mkdir(parents=True)
...
def test_find_dataset_reads_a_feat_l0_from_centered_archive(tmp_path: Path) -> None:
    """Selector 应直接读取 centered A_feat_L0，不再二次读取 receptor_tokens。"""

    outputs = tmp_path / "stage1_outputs"
    upstream = tmp_path / "upstream"
    _write_unet_pdb(outputs, upstream, "train", "1abc", model_name="Find_1")
    centered_path = outputs / "Find_1" / "train" / "1abc" / "centered" / "CLG_centered.npz"
    with np.load(centered_path, allow_pickle=False) as source:
        centered = {name: np.asarray(source[name]) for name in source.files}
    expected_l0 = np.arange(2 * 49, dtype=np.float32).reshape(2, 49)
    centered.update(
        {
            "P_offsets": np.asarray([0, 1], dtype=np.int64),
            "P_coord_local_xyz": np.asarray([[1.5, 1.5, 1.5]], dtype=np.float32),
            ...
            "A_global_index": np.asarray([4, 1], dtype=np.int64),
            "A_coord_local_xyz": np.asarray([[1.5, 1.5, 1.5], [2.5, 2.5, 2.5]], dtype=np.float32),
            "A_coord_centered_world": np.asarray([[-38.5] * 3, [-37.5] * 3], dtype=np.float32),
            "A_probability": np.asarray([0.8, 0.6], dtype=np.float32),
            "A_feat_L0": expected_l0,
            "A_feat_L1": np.ones((2, 2), dtype=np.float16),
            "A_feat_L2": np.ones((2, 3), dtype=np.float16),
            "A_feat_L3": np.ones((2, 4), dtype=np.float16),
            "candidate_A_offsets": np.asarray([0, 2, 3, 4], dtype=np.int64),
            "candidate_A_index": np.asarray([0, 1, 0, 1], dtype=np.int32),
        }
    )
    np.savez_compressed(centered_path, **centered)

    frozen = freeze_input_clg_list(
        stage1_outputs_root=outputs,
        selector_run_dir=tmp_path / "run_find",
    ...
    np.testing.assert_array_equal(dataset[0]["A_sources"]["A_feat_L0"].numpy(), expected_l0)
    assert not (upstream / "parse" / "1abc" / "receptor_tokens.npz").exists()
