...
def _build_v_only_model() -> CandidateConditionedLineageNetwork:
    ...
    return CandidateConditionedLineageNetwork(
        ...
        v_output_dim=48,
        v_grid_source_order=("d2", "d3", "d4", "c4"),
        v_grid_source_dims={"d2": 4, "d3": 4, "d4": 4, "c4": 4},
        v_meta_dim=2,
        v_fusion_projected_dim=8,
        v_fusion_gate_hidden_dim=16,
...
def _v_only_sample() -> dict:
    ...
    return {
        ...
        "V_native_grids": {
            "d2": torch.randn(1, 4, 4, 4, 4),
            "d3": torch.randn(1, 4, 2, 2, 2),
            "d4": torch.randn(1, 4, 2, 2, 2),
            "c4": torch.randn(1, 4, 2, 2, 2),
        },
...
def test_v48_fallback_ignores_dataset_multiscale_and_density_artifacts() -> None:
    """关闭 m/c 后允许 Dataset 保持统一读取，模型只消费 V48 基础表。"""
    model = CandidateConditionedLineageNetwork(
        modalities=("V",),
        a_source_order=(),
        a_source_dims={},
