...
因此，比较器不再把共同路径的逐字节相等当作科学契约。它仍要求第一个 optimizer step 前的 353 个共同参数逐字节相同，并把后续共同输出、损失、梯度、裁剪系数、参数更新和 AdamW 状态限制在 `compare_optimization_traces.py` 中冻结的字段化包络内。相对差和传统绝对差上限以 attempt a6 的观测上界为基准保留有限余量；attempt a6 不含稠密旁车，只能说明逐字节相等不是可行契约，不能被追认为最终通过。

schema 4 的稠密比较由 attempt a7 采集八条完整轨迹，再由 attempt a8 以相同模型角色、相同请求和不同进程的自然运行作为测量底噪。a8 是只读标定，不是验收通过：

- AUTO trunk 同角色重复的体素输出逐元素最大绝对差为 `2.31640625`，完整模型同角色重复为 `2.5`；跨角色 replay 最大值为 `2.4375`。
- AUTO trunk 同角色重复的参数更新逐元素最大绝对差为 `3.9689243e-5`，完整模型同角色重复为 `3.9629638e-5`；跨角色最大值为 `3.9698556e-5`。
- 完整模型同角色重复在单个 microbatch 的累计体素梯度 `l2` 相对差 P95 最大值为 `8.4122e-2`，跨角色最大值为 `8.8515e-2`。
- 完整模型同角色重复在单个 microbatch 的累计体素梯度最大值相对差 P95 为 `1.2081e-1`，跨角色最大值为 `1.4286e-1`。

基于同角色底噪并留出有限余量，schema 4 首先把体素输出逐元素绝对差上限设为 `3.0`。独立 attempt a9 的受控比较通过，但 trunk 序列 replay 在第 8 个 microbatch 观测到单点差 `3.076171875`；其余身份、recycle 和数值门禁全部通过。为避免把约 2.5% 的边缘越界误判为结构差异，该上限最终冻结为 `3.5`，并要求用新进程重新生成受影响的 replay 对进行独立复核。累计体素梯度 `l2` 相对差 P95、累计体素梯度最大值相对差 P95 和参数更新逐元素绝对差仍分别为 `0.10`、`0.16` 和 `5e-5`；其余上限不变。a7、a8 与 a9 均不得因参与标定而被追认为最终通过。
