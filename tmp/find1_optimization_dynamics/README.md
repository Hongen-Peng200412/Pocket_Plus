...
基于同角色底噪并留出有限余量，schema 4 冻结四项新上限：体素输出逐元素绝对差 `3.0`，累计体素梯度 `l2` 相对差 P95 `0.10`，累计体素梯度最大值相对差 P95 `0.16`，参数更新逐元素绝对差 `5e-5`。其余上限不变。随后必须以新的独立八轨迹 attempt 验证整套比较器；a7 与 a8 不得因参与标定而被追认为最终通过。
